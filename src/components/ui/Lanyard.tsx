...
export default function Lanyard({
  ...
  const handleHoverChange = (isHovering: boolean) => {
    if (containerRef.current) {
      containerRef.current.style.pointerEvents = isHovering ? 'auto' : 'none';
    }
  };
...
function Band({
  ...
  useEffect(() => {
    if (onHoverChange) {
      onHoverChange(hovered || hoveredCopy);
    }
    if (hoveredCopy) {
  ...
  }, [hovered, dragged, hoveredCopy, onHoverChange]);
  ...
  return (
    <>
      <group position={[isMobile ? viewport.width / 2 - 1.5 : viewport.width / 2 - 3, viewport.height / 2 + 1, 0]}>
            ...
            <mesh 
              ...
              onPointerMove={(e) => {
                if (e.uv) {
                  const u = e.uv.x;
                  const v = e.uv.y;
                  if (u > 0.41 && u < 0.48 && ((v > 0.58 && v < 0.70) || (v > 0.30 && v < 0.42))) {
                    setHoveredCopy(true);
                  } else {
              ...
              onPointerDown={(e) => {
                if (e.uv) {
                  ...
                  if (u > 0.41 && u < 0.48) {
                    // Email
                    if ((v > 0.58 && v <= 0.64) || (v > 0.36 && v < 0.42)) {
                      e.stopPropagation();
                      navigator.clipboard.writeText('3187263951@qq.com');
                      alert('邮箱已复制！');
                      return;
                    }
                    // Phone
                    if ((v > 0.64 && v < 0.70) || (v > 0.30 && v <= 0.36)) {
                      e.stopPropagation();
                      navigator.clipboard.writeText('151 1191 4008');
                      alert('电话已复制！');
                      return;
                    }
                  }
